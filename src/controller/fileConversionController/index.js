...
const fileConverterController = {
  ...
  getImages: async (req, res) => {
    try {
      ...
      if (!fs.existsSync(folderPath)) return res.status(404).json({ error: "No images found" });
  
      const imageFiles = fs.readdirSync(folderPath).filter(file => file.endsWith(".png"));
      const imageUrls = imageFiles.map(file => `/uploads/pdf-images/${pdfName}/${file}`);
...
async function convertPDFToImages(pdfPath, pdfName) {
  ...
  return fs.readdirSync(outputPath)
    ...
    .map(file => `/uploads/pdf-images/${pdfName}/${file}`);
}